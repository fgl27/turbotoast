...
//                        RootUtils.runCommand("ls " + Constants.BATTERY_PARAMETERS) : listfiles(Constants.BATTERY_PARAMETERS));
        ...
        git_summary.setOnClickListener(v -> {
            try {
                startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse(git_link)));
            } catch (ActivityNotFoundException ex) {
                Tools.DoAToast(getString(R.string.no_browser), AboutContext);
...
//        StringBuilder resultList = new StringBuilder("No Root" + "\n");
//        File[] fList = directory.listFiles();
//        if (fList != null) {
//            for (File file: fList) {
//                if (file.isFile()) {
//                    resultList.append(file.getName()).append("\n");
//                }
//            }
//        } else resultList = new StringBuilder("Can't read Power status");
